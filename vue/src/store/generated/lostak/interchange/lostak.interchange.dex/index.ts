...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: lostak.interchange.dex initialized!')
	...
	gas: "200000" }, memo})
		...
		async sendMsgPort({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgPort(value)
				const result = await txClient.signAndBroadcast([msg], {fee: { amount: fee, 
	gas: "200000" }, memo})
				return result
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgPort:Init Could not initialize signing client. Wallet is required.')
				}else{
					throw new Error('TxClient:MsgPort:Send Could not broadcast Tx: '+ e.message)
				}
			}
		},
	...
	gas: "200000" }, memo})
		...
		async MsgPort({ rootGetters }, { value }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgPort(value)
				return msg
				...
				if (e == MissingWalletError) {
					...
					throw new Error('TxClient:MsgPort:Create Could not create message: ' + e.message)
				}
			}
		},
